import os, shutil, glob, calendar
...
def load_schedule():
    if not PAY_PERIOD_SCHEDULE_CSV.exists(): return None
...
def find_header_row(df_no_header, expected_headers):
    for i in range(min(20, len(df_no_header))):
        ...
        if (sum(fuzz.partial_ratio(h, row_str) for h in expected_headers) / len(expected_headers)) > 80: return i
    return 14
...
def main():
    ...
    if schedule is None: print("Error: pay_periods.csv not found."); return
    ...
    if report1 and report2:
        ...
        if not pp_details: print(f"Error: No Pay Period found for date {date_str}."); return
        try:
            if merge_reports(report1_path, report2_path, pp_details):
                os.remove(report1_path); os.remove(report2_path)
                print("Process complete.")
        except Exception as e: print(f"ERROR during merge: {e}")
    else: print("Did not find both required report files in landing zone.")
